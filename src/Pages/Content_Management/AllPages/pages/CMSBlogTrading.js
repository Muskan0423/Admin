...
import { getAboutUs_cms, getUserLogin, getContactPage_cms, getHomePage_cms, getAllBlogs_cms, deleteBlog_cms, getAllBlogs_cms_trading, deleteBlog_cms_trading } from "../../../User_Management/features/userSlice";
...
const Action = ({ blog_id }) => {
  ...
  const handleClick = async () => {
    ...
    Navigate('/home/editblogtrading', { state: data })
  };
  ...
  const handleConfirmDelete = async () => {
    // console.log(blog_id)
    await dispatch(deleteBlog_cms_trading(blog_id))
      .then(() => {
...
const CMSBlogTrading = ({ setActiveTab, setExpand }) => {
  setExpand("contentManagement");
  setActiveTab("blogs");
  const head = "Blogs for trading";
  const [searchTerm, setSearchTerm] = useState("");

  const dispatch = useDispatch();
  const navigate = useNavigate();
  const blogData = useSelector((state) => state.userManagement.getAllBlogs_cms_trading)
  const blogData12 = useSelector((state) => state.userManagement?.getAllBlogs_cms_trading)
  console.log(blogData12,"skdhjvbsdvhb");

  useEffect(() => {
    const fetchBlogData = async () => {
      setLoading(true);
      await dispatch(getAllBlogs_cms_trading());
      setLoading(false);
    };
    fetchBlogData();
  }, [dispatch]);
  ...
  const greenClicked = () => {
    navigate("/home/addblogfortrading");
  };
